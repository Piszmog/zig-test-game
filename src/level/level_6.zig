...
pub fn health(renderer: sdl.Renderer) !void {
    var body_1 = sdl.RigidBody.init(sdl.Rect.init(320, 200, 10, 10, sdl.Color{ .red = 0xff, .green = 0, .blue = 0, .alpha = 0xff }));
    var char_1 = sdl.Character.init(sdl.RigidBody.init(sdl.Rect.init(400, 200, 10, 10, sdl.Color{ .red = 0xff, .green = 0, .blue = 0, .alpha = 0xff })));
    ...
    mainloop: while (true) {
        ...
        while (i < projectiles.items.len) {
            if (projectiles.items[i].rect.sdl_rect.x > body_1.rect.sdl_rect.x + 300) {
                _ = projectiles.swapRemove(i);
                continue;
            }

            if (char_1.health > 0 and sdl.has_collision(projectiles.items[i], char_1.body)) {
                char_1.health -= 10;
                _ = projectiles.swapRemove(i);
                continue;
            }
        ...
        if (char_1.health > 0) {
            try renderer.setDrawColor(char_1.body.rect.color);
            try renderer.fillRect(char_1.body.rect);
        }
